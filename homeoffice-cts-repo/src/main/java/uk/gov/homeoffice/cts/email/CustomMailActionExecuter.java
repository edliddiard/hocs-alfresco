...
public class CustomMailActionExecuter extends MailActionExecuter {
private static Log logger = LogFactory.getLog(MailActionExecuter.class);
    ...
    public static final String PARAM_ATTACH_RESPONSE = "attachResponse";
	...
	public MimeMessageHelper prepareEmail(final Action ruleAction , final NodeRef actionedUponNodeRef, final Pair<String, Locale> recipient, final Pair<InternetAddress, Locale> sender){
        ...
        MimeMessagePreparator mailPreparer = new MimeMessagePreparator()
        {
            ...
            public void prepare(MimeMessage mimeMessage) throws MessagingException
            {
                ...
                String addContent = (String)ruleAction.getParameterValue(PARAM_ATTACH_RESPONSE);
                if(addContent != null && addContent.equals("true")) {
                    addContent(actionedUponNodeRef, mimeMessage, text);
                }
    ...
    protected void addContent(NodeRef actionedUponNodeRef, MimeMessage mimeMessage, String text) throws MessagingException {
        Map<QName,Serializable> props = nodeService.getProperties(actionedUponNodeRef);
        //only attach content for TRO and MIN when at Dispatch to

        if( props.get(CtsModel.PROP_CORRESPONDENCE_TYPE).equals(CorrespondenceType.DCU_TREAT_OFFICIAL.getCode()) || props.get(CtsModel.PROP_CORRESPONDENCE_TYPE).equals(CorrespondenceType.DCU_MINISTERIAL.getCode()) ) {
            ...
            //get the document in this case
            List<ChildAssociationRef> childAssocs = nodeService.getChildAssocs(actionedUponNodeRef, ContentModel.ASSOC_CONTAINS, RegexQNamePattern.MATCH_ALL);
            for (Iterator iterator = childAssocs.iterator(); iterator
                    .hasNext(); ) {
                ChildAssociationRef childAssociationRef = (ChildAssociationRef) iterator
                        .next();
                final NodeRef documentNodeRef = childAssociationRef.getChildRef();
                //check if it is a response document
                Serializable docType = nodeService.getProperty(documentNodeRef, CtsModel.PROP_DOCUMENT_TYPE);
                logger.debug("Doctype:" + docType);
                if (docType != null && docType.equals("Response")) {
                    MimeBodyPart attachment = new MimeBodyPart();
                    attachment.setFileName(nodeService.getProperty(documentNodeRef, ContentModel.PROP_NAME).toString());
                    attachment.setDataHandler(new DataHandler(new DataSource() {
                        public InputStream getInputStream() throws IOException {
                            ContentReader reader = serviceRegistry.getContentService().getReader(documentNodeRef, ContentModel.PROP_CONTENT);
                        ...
                        public String getContentType() {
                            return serviceRegistry.getContentService().getReader(documentNodeRef, ContentModel.PROP_CONTENT).getMimetype();
                        }

                        public String getName() {
                            return nodeService.getProperty(documentNodeRef, ContentModel.PROP_NAME).toString();
                        }
                    }));
