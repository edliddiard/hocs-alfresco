 ...
 * See org/alfresco/repo/web/scripts/comments/CommentsPost.java
 * This class is the controller for the comments.post web script.
 *
 * Created by dawud rahman on 28/04/2016.
 */

public class CommentsWebScript extends AbstractCommentsWebScript {

    private static final Logger LOGGER = LoggerFactory.getLogger(CommentsWebScript.class);
    ...
    protected Map<String, Object> executeImpl(NodeRef nodeRef, WebScriptRequest req, Status status, Cache cache)
    {
        ...
        LOGGER.debug("Processing Add Comments - Request parameters: [{}]", json.toJSONString());
    ...
    private NodeRef addComment(NodeRef nodeRef, JSONObject json)
    {
        ...
        // fetch the content nodes children
        JSONObject jsonChild = (JSONObject) json.get(JSON_KEY_CONTENT);
    ...
    {
        if (hasAspect(nodeRef, ForumModel.ASPECT_DISCUSSABLE))
        {
    ...
    public String sanitize(String content) {
        ...
        if(content!=null && !content.isEmpty()) {
            // Sanitize input to remove script and make safe
            //case 1 : <script> are removed
            //case 2 : javascript: call are removed
            //case 3 : remove on* attributes like onLoad or onClick
            //case 4 : a special regular expression is used to strip out anything between a < and >
