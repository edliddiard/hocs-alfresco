 ...
 * @method GET
 *
 * URL: /alfresco/service/permissions/{store_type}/{store_id}/{id}/{setOrRemove}/{permission}/{authority}
 *
 ...
 *
 * setOrRemove: Uses either setPermission() or removePermission()
 *
 * permission: Permission to either be set or removed
 *
 * authority: Authority related to the permission above
 *
...
if (url.templateArgs.permission == undefined) {
	error = true;
	status.code = 400;
	status.message = "Please enter a valid permission.";
	status.redirect = true;
}

if (url.templateArgs.authority == undefined) {
	error = true;
	status.code = 400;
	status.message = "Please enter a valid authority.";
	status.redirect = true;
}
...
if (!error) {
	...
	model.before = node.getPermissions();

	if (url.templateArgs.setOrRemove == "remove") {
		node.removePermission(url.templateArgs.permission, url.templateArgs.authority);
	} else if (url.templateArgs.setOrRemove == "set") {
		node.setPermission(url.templateArgs.permission, url.templateArgs.authority);
	}
